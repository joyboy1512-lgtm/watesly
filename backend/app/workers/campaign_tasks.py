...
from app.services.meta_reliability import is_transient_meta_error, sleep_for_meta_backoff
...
async def _run_campaign(campaign_id: UUID, execution_token: UUID) -> dict:
    async with AsyncSessionFactory() as db:
        ...
        if campaign.execution_token != execution_token:
            return {"status": "stale_task"}
        if campaign.requires_approval and not campaign.approved_at:
            return {"status": "blocked", "reason": "approval_required"}
        if campaign.status == CampaignStatus.CANCELLED:
            return {"status": "cancelled"}

        wa = await db.get(WhatsAppAccount, campaign.whatsapp_account_id)
        template = await db.get(WhatsAppTemplate, campaign.template_id)
        if not wa or not template:
        ...
        while True:
            ...
            if campaign.status in {CampaignStatus.PAUSED, CampaignStatus.CANCELLED}:
                return {"status": campaign.status.value, "sent": sent, "failed": failed}
            ...
            await db.execute(
                ...
                .where(
                    ...
                    CampaignRecipient.status == CampaignRecipientStatus.SENDING,
                    CampaignRecipient.sending_started_at < cutoff,
                    CampaignRecipient.external_message_id.is_(None),
                )
                .values(
                    status=CampaignRecipientStatus.FAILED,
                    error_message="Delivery state unknown after worker interruption; reconcile before retry",
            ...
            for recipient, contact in rows:
                await db.refresh(campaign)
                if campaign.execution_token != execution_token or campaign.status in {
                ...
                recipient.delivery_key = (
                    recipient.delivery_key or f"campaign:{campaign.id}:recipient:{recipient.id}"
                )
                campaign.last_heartbeat_at = now
                await db.commit()
                ...
                try:
                    components = resolve_send_components(
                        template.components, recipient.template_parameters
                    )
                    ...
                    if is_transient_meta_error(exc):
                        recipient.status = CampaignRecipientStatus.QUEUED
                        recipient.error_message = str(exc)[:2000]
                        rate_limited += 1
                        meta_attempt += 1
                        await db.commit()
                        await sleep_for_meta_backoff(exc, attempt=meta_attempt)
                        continue
                    recipient.status = CampaignRecipientStatus.FAILED
                    recipient.error_message = str(exc)[:2000]
                    from app.services.contact_reachability import record_delivery_failure
        ...
        campaign.status = (
            CampaignStatus.COMPLETED if failed == 0 else CampaignStatus.COMPLETED_WITH_ERRORS
        )
